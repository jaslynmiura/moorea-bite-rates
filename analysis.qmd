...
var_total_bites > mean_total_bites
```
...
We want to use an offset since larger or smaller data could be collected based on the time spent measuring total bite counts.
...
ggplot(bite_pred_grid, aes(size_cm, predicted_bites, color = species)) +
  ...
  labs(x = "Length of Fish (cm)",
       y = "Total bite counts",
       color = "Species") + ## Would this be the number of bites or the predicted number of bites
  theme_minimal()
```
